...
int print_network(deque<set<int> > &E, const deque<deque<int> > &member_list,
                  ...
                  deque<int> &num_seq, deque<map<int, double> > &neigh_weigh, double beta,
  ...
  for (int i = 0; i < member_matrix.size(); i++) {
    ...
    for (int j = 0; j < member_matrix[i].size(); j++) {
      ...
      media_est += E[member_matrix[i][j]].size() -
                   double(internal_kin_only_one(E[member_matrix[i][j]], member_matrix[i]));
  ...
  cout << "\naverage mixing parameter (topology): " << average_func(double_mixing) << " +/- "
  ...
  statout << "degree distribution (probability density function of the degree in logarithmic bins) "
          << endl;
  ...
  for (int i = 0; i < neigh_weigh.size(); i++) {
    ...
    for (map<int, double>::iterator itm = neigh_weigh[i].begin();
         itm != neigh_weigh[i].end(); itm++) {
  ...
  cout << "\naverage mixing parameter (weights): " << average_func(one_minus_mu2) << " +/- "
       << sqrt(variance_func(one_minus_mu2)) << endl;
  ...
  cout << "average weight of an internal link " << average_func(inwij) << " +/- "
       << sqrt(variance_func(inwij)) << endl;
  cout << "average weight of an external link " << average_func(outwij) << " +/- "
       << sqrt(variance_func(outwij)) << endl;
...
int check_weights(deque<map<int, double> > &neigh_weigh, const deque<deque<int> > &member_list,
                  ...
                  const double tot_var, double *strs) {
  ...
  for (int k = 0; k < member_list.size(); k++) {
    ...
    for (map<int, double>::iterator itm = neigh_weigh[k].begin();
         itm != neigh_weigh[k].end(); itm++) {
...
int propagate(deque<map<int, double> > &neigh_weigh, const deque<deque<int> > &member_list,
              ...
              double *strs, const deque<int> &internal_kin_top) {
  ...
  {    // in this case I rewire the idle strength
    ...
    for (map<int, double>::iterator itm = neigh_weigh[i].begin();
      ...
      if (itm->second + change > 0)
        for (int bw = 0; bw < 3; bw++)
          oldpartvar += (factual[itm->first][bw] - wished[itm->first][bw]) *
    ...
    for (map<int, double>::iterator itm = neigh_weigh[i].begin();
      ...
      if (itm->second + change > 0) {
        ...
        for (int bw = 0; bw < 3; bw++)
          newpartvar += (factual[itm->first][bw] - wished[itm->first][bw]) *
                        (factual[itm->first][bw] - wished[itm->first][bw]);
...
int weights(deque<set<int> > &en, const deque<deque<int> > &member_list, const double beta,
            const double mu, deque<map<int, double> > &neigh_weigh) {
...
int benchmark(bool excess, bool defect, int num_nodes, double average_k,
              int max_degree, double tau, double tau2,
              double mixing_parameter, double mixing_parameter2, double beta,
              int overlapping_nodes, int overlap_membership, int nmin,
              int nmax, bool fixed_range, double ca,
              deque<set<int> > &output_edge_list,
  ...
  for (int i = 0; i < num_nodes; i++) {
    int nn =
    lower_bound(cumulative.begin(), cumulative.end(), ran4()) - cumulative.begin() + min_degree;
  ...
  if (
  internal_degree_and_membership(mixing_parameter, overlapping_nodes, overlap_membership, num_nodes,
  ...
  if (
  build_subgraphs(E, member_matrix, member_list, link_list, internal_degree_seq, degree_seq, excess,
                  defect) == -1)
    return -1;
  ...
  output_edge_list = std::move(E);
  output_member_list = std::move(member_list);
  output_weights = std::move(neigh_weigh);
...
void build_network(int num_nodes, double average_k, int max_degree, double tau,
  ...
  benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree, p.tau,
            p.tau2, p.mixing_parameter, p.mixing_parameter2, p.beta,
            p.overlapping_nodes, p.overlap_membership, p.nmin, p.nmax,
            p.fixed_range, p.clustering_coeff, output_edge_list,
            output_member_list, output_weights);
}